...
@section('id_nav', 'clients_nav')

@section('content')
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            @include('admin.elements.h1', ['page' => 'Klien'])
                        ...
                        <tbody>
                            @foreach ($data as $item => $value)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="p-4">
                                        <img src="{{ asset('assets/template/assets/img/clients/'.$value->logo) }}" class="w-16 md:w-32 max-w-full max-h-full" alt="Apple Watch">
                                    </td>
                                    <td class="px-6 py-4">
    ...
    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form class="p-4 md:p-5 disabled-btn"  action="{{ route('clients.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="col-span-1">
                        @include('admin.elements.label', ['text' => 'Logo'])
                        @include('components.input-file', ['name' => 'logo', 'accept' => '.jpg,.jpeg', 'type' => 'JPG or JPEG'])
                    </div>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah kamu yakin ingin menghapus data?</h3>
                        <form action="{{ route('clients.destroy', ['id' => $value->id]) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
        ...
        <div id="m-upd{{ $item }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <form class="p-4 md:p-5 disabled-btn"  action="{{ route('clients.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

...
@section('id_nav', 'testimonials_nav')

@section('content')
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            @include('admin.elements.h1', ['page' => 'Testimoni'])
                            ...
                            @foreach ($testimonials as $item => $value)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                        <img class="w-10 h-10 rounded-full" src="{{ asset('assets/images/testimonial/' . $value->photo) }}" alt="Jese image">
                                        <div class="ps-3">
                                            <div class="text-base font-semibold">{{ $value->name }}</div>
                                            <div class="font-normal text-gray-500">{{ $value->position }}</div>
                                        </div>
                                    </th>
                                    <td class="px-6 py-4">
                                        {{ $value->text }}
    ...
    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form class="p-4 md:p-5" action="{{ route('testimonials.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="col-span-2">
                        ...
                        <x-input-text :name="'position'" :placeholder="'...'" />
                    </div>
                    <div class="col-span-2">
                        @include('admin.elements.label', ['text' => 'Photo'])
                        <x-input-file :name="'photo'" :accept="'.jpg,.jpeg'" :type="'JPG or JPEG.'" />
                    </div>
                    <div class="col-span-2">
                        @include('admin.elements.label', ['text' => 'Testinomi'])
                        <textarea required name="text" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
                    </div>
                    <x-button />
                </form>
            </div>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <form action="{{ route('testimonials.destroy', ['id' => $value->id]) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
        ...
        <div id="m-upd{{ $item }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <form class="p-4 md:p-5" action="{{ route('testimonials.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="col-span-2">
                            @include('admin.elements.label', ['text' => 'Pilih Klien'])
                            ...
                            <select name="clients_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                @foreach ($clients as $items)
                                    <option {{ $items->id == $value->clients_id ? "selected" : "" }} value="{{ $items->id }}">{{ $items->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-span-2">
                            <x-label :text="'Nama'" />
                            <x-input-text :value="$value->name" :name="'name'" :placeholder="'...'" />
                        </div>
                        <div class="col-span-2">
                            <x-label :text="'Position'" />
                            <x-input-text :value="$value->position" :name="'position'" :placeholder="'...'" />
                        </div>
                        <div class="col-span-2">
                            @include('admin.elements.label', ['text' => 'Photo'])
                            <x-input-file :name="'photo'" :accept="'.jpg,.jpeg'" :type="'JPG or JPEG.'" />

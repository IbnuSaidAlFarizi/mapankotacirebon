...
@section('content')
    ...
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            @include('admin.elements.h1', ['page' => 'Tag Berita'])
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <form action="{{ route('news.tag.destroy', ['id' => $value->id]) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
    ...
    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form class="p-4 md:p-5" action="{{ route('news.tag.store') }}" method="POST">
                    @csrf
                    @method('POST')
                    @include('admin.elements.label', ['text' => 'Nama Tag'])
                    <x-input-text :name="'title'" :placeholder="' '" />
                    @include('admin.elements.label', ['text' => 'Meta Keyword'])
                    <x-input-text :name="'keyword'" :placeholder="' '" />
                    @include('admin.elements.label', ['text' => 'Meta Description'])
                    <x-textarea :name="'description'" :value="''" :placeholder="'Ringkasan mengenai kja Anda'" />
                    <x-button />
                </form>
            </div>
        </div>
            ...
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="p-4 md:p-5" action="{{ route('news.tag.update') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="id" value="{{ $value->id }}">
                            <div class="grid gap-4 mb-4 grid-cols-1">
                                @include('admin.elements.label', ['text' => 'Nama Kategori'])
                                <x-input-text :name="'title'" :placeholder="' '" :value="$value->title" />
                            </div>
                            <div class="grid gap-4 mb-4 grid-cols-1">
                                @include('admin.elements.label', ['text' => 'Meta Keyword'])
                                <x-input-text :name="'keyword'" :placeholder="' '" :value="$value->keyword" />
                            </div>
                            <div class="grid gap-4 mb-4 grid-cols-1">
                                @include('admin.elements.label', ['text' => 'Meta Description'])
                                <x-textarea :name="'description'" :value="$value->description" :placeholder="'Ringkasan mengenai kja Anda'" />
                            </div>
                            <x-button-update />
                        </form>
                    </div>

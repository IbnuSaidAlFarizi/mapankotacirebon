...
@section('id_nav', 'services_nav')

@section('content')
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            <x-jumbroton :page="'Layanan    '" :url="'services'" class="mt-2" />
            <div class="grid md:grid-cols-1 gap-8">
                <x-form-meta :url="'meta-services.update'" :meta="$meta" />
                <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-8 md:p-12">
                    <x-tag-icon :page="'Halaman Beranda'" />
                    <form action="{{ route('services.update_hero') }}" method="POST">
                        @csrf
                        @method('PUT')
                        @csrf
                        <x-label :text="'Judul'" />
                        <x-input-text :name="'h1'" :value="$services->h1" :placeholder="'cth: Website KJA, KJA ABC...'" />
                        <x-label :text="'Deskripsi'" />
                        <x-textarea :name="'p'" :value="$services->p" :placeholder="'Ringkasan mengenai kja Anda'" />
                        <x-button />
                    </form>
                </div>
                ...
                <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 w-full max-w-2xl max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <!-- Modal body -->
                            <form class="p-4 md:p-5" action="{{ route('services.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('POST')
                                <div class="grid gap-4 mb-4 grid-cols-2">
                                    <div class="col-span-2">
                                        <x-label :text="'Meta Keyword'" />
                                        <x-input-text :name="'keyword'" :placeholder="'cth: Layanan KJA, Pembukuan dll...'" />
                                        <p id="filled_success_help" class=" text-xs text-blue-600 dark:text-blue-400">Pisahkan dengan koma</p>
                                    </div>
                                    <div class="col-span-2">
                                        <x-label :text="'Meta Description'" />
                                        <x-input-text :name="'description'" :placeholder="'Ringkasan mengenai layanan Anda'" />
                                    </div>
                                    <div class="col-span-2">
                                        <hr class="m-0 p-0">
                                    </div>
                                    <div class="col-span-1">
                                        <x-label :text="'Nama Layanan'" />
                                        <x-input-text :name="'name'" :placeholder="'...'" />
                                    </div>
                                    <div class="col-span-1">
                                        <x-label :text="'Ikon'" />
                                        <x-input-text :name="'icon'" :placeholder="'...'" />
                                        <a href="https://v2.lineicons.com/" target="blank" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Cari ikon</a>
                                    </div>
                                    <div class="col-span-2">
                                        <x-label :text="'Foto'" />
                                        <x-input-file required :name="'photo'" :accept="'.jpg,.jpeg'" :type="'JPG or JPEG.'" />
                                    </div>
                                    <div class="col-span-2">
                                        <x-label :text="'Konten'" />
                                        <x-textarea-summernote :name="'content'" />
                                    </div>
                                </div>
                                <x-button />
                            </form>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <form action="{{ route('services.destroy', ['id' => $value->id]) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5">
                    <form class="p-4 md:p-5" action="{{ route('services.update') }}" method="POST"  enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                ...
                                <x-input-text :name="'keyword'" :value="$value->keyword" :placeholder="'cth: Layanan KJA, Pembukuan dll...'" />
                                <p id="filled_success_help" class=" text-xs text-blue-600 dark:text-blue-400">Pisahkan dengan koma</p>
                            </div>
                            <div class="col-span-2">
                                <x-label :text="'Meta Description'" />
                                <x-input-text :name="'description'" :value="$value->description" :placeholder="'Ringkasan mengenai layanan Anda'" />
                            </div>
                            <div class="col-span-2">
                                <hr class="m-0 p-0">
                            </div>
                            <div class="col-span-1">
                                <x-label :text="'Nama Layanan'" />
                                <x-input-text :name="'name'" :value="$value->name" :placeholder="'...'" />
                            </div>
                            <div class="col-span-1">
                                <x-label :text="'Ikon'" />
                                <x-input-text :name="'icon'" :value="$value->icon" :placeholder="'...'" />
                                <a href="https://v2.lineicons.com/" target="blank" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Cari ikon</a>
                            </div>
                            <div class="col-span-2">
                                <x-label :text="'Foto'" />
                            ...
                            <div class="col-span-2">
                                <x-label :text="'Konten'" />
                                <x-textarea-summernote :name="'content'" :value="$value->content" />
                            </div>
                        </div>
                        <x-button-update />
                    </form>

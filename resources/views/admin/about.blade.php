...
@section('id_nav', 'about_nav')

@section('content')
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            <x-jumbroton :page="'Tentang Kami'" :url="'tentang-kami'" class="mt-2" />
            <div class="grid md:grid-cols-1 gap-8">
                <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-8 md:p-12">
                    <x-tag-icon :page="'Halaman Tentang Kami'" />
                    <form action="{{ route('about.update') }}"  class="disabled-btn"  method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                @include('components.input-text', ['name' => 'title', 'value' => $about->title, 'placeholder' => 'Judul'])
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                ...
                                @include('admin.elements.textarea-summernote', ['name' => 'description', 'value' => $about->description])
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Gambar'])
                                @include('components.input-file', ['name' => 'section1_image', 'accept' => '.jpg,.jpeg', 'type' => 'JPG or JPEG'])
                            </div>
                        </div>
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Deskripsi'])
                                @include('admin.elements.textarea-summernote', ['name' => 'section1_description', 'value' => $about->section1_description])
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Video'])
                                @include('components.input-file', ['name' => 'section2_video', 'accept' => '.mp4', 'type' => 'MP4'])
                            </div>
                            <div>
                                @include('components.label', ['text' => 'Gambar'])
                                @include('components.input-file', ['name' => 'section2_image', 'accept' => '.jpg,.jpeg', 'type' => 'JPG or JPEG'])
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Deskripsi'])
                                @include('admin.elements.textarea-summernote', ['name' => 'section2_description', 'value' => $about->section2_description])
                            </div>
                        </div>
                    ...
                    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            ...
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <form class="p-4 md:p-5 disabled-btn"  action="{{ route('whyus.store') }}" method="POST">
                                    @csrf
                                    @method('POST')
                                    <div class="grid gap-4 mb-4 grid-cols-1">
                                        @include('admin.elements.label', ['text' => 'Judul'])
                                        @include('components.input-text', ['name' => 'title', 'value' => '', 'placeholder' => 'Judul'])
                                    </div>
                                    <div class="grid gap-4 mb-4 grid-cols-1">
                                        @include('admin.elements.label', ['text' => 'Deskripsi'])
                                        @include('components.textarea', ['name' => 'description', 'value' => '', 'placeholder' => 'Deskripsi'])
                                    </div>
                                    @include('components.button')
                                </form>
                            </div>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah kamu yakin ingin menghapus data {{ $value->title }}?</h3>
                        <form action="{{ route('whyus.destroy', ['id' => $value->id]) }}" method="POST" class="inline disabled-btn" >
                            @csrf
                            @method('DELETE')
            ...
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="p-4 md:p-5 disabled-btn"  action="{{ route('whyus.update', ['id' => $value->id]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="id" value="{{ $value->id }}">
                            <div class="grid gap-4 mb-4 grid-cols-1">
                                @include('admin.elements.label', ['text' => 'Judul'])
                                @include('components.input-text', ['name' => 'title', 'value' => $value->title, 'placeholder' => 'Judul'])
                            </div>
                            <div class="grid gap-4 mb-4 grid-cols-1">
                                @include('admin.elements.label', ['text' => 'Deskripsi'])
                                @include('components.textarea', ['name' => 'description', 'value' => $value->description, 'placeholder' => 'Deskripsi'])
                            </div>
                            @include('components.button')
                        </form>
                    </div>

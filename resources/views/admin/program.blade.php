...
@section('title', 'Halaman Program Unggulan')
@section('id_nav', 'program_nav')

@section('content')
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            <x-jumbroton :page="'Program Unggulan'" :url="'program-unggulan'" class="mt-2" />
            <div class="grid md:grid-cols-1 gap-8">
                <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-8 md:p-12">
                    <x-tag-icon :page="'Halaman Program Unggulan'" />
                    <form action="{{ route('program.update_hero') }}" class="disabled-btn" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Judul'])
                                @include('components.input-text', ['name' => 'title', 'value' => $program->title, 'placeholder' => 'Judul'])
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                ...
                                @include('admin.elements.textarea-summernote', ['name' => 'description', 'value' => $program->description])
                            </div>
                        </div>
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Info Beasiswa'])
                                @include('admin.elements.textarea-summernote', ['name' => 'info_beasiswa', 'value' => $program->info_beasiswa])
                            </div>
                        </div>
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Quotes'])
                                @include('admin.elements.textarea', ['name' => 'quote', 'value' => $program->quote])
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Nama Admin'])
                                @include('components.input-text', ['name' => 'admin_name', 'value' => $program->admin_name, 'placeholder' => 'Nama Admin'])
                            </div>
                            <div>
                                @include('components.label', ['text' => 'Posisi'])
                                @include('components.input-text', ['name' => 'admin_position', 'value' => $program->admin_position, 'placeholder' => 'Posisi'])
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                            ...
                            <div>
                                @include('components.label', ['text' => 'URL Pendaftaran'])
                                @include('components.input-text', ['name' => 'link_pendaftaran', 'value' => $program->link_pendaftaran, 'placeholder' => 'https://'])
                            </div>
                        </div>
                ...
                <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-8 md:p-12">
                    <x-tag-icon :page="'Program Unggulan'" />
                    ...
                    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative p-4 w-full max-w-xl max-h-full">
                            ...
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <form class="p-4 md:p-5 disabled-btn" action="{{ route('program.store') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="grid gap-4 mb-4 grid-cols-1">
                                        <div>
                                            @include('admin.elements.label', ['text' => 'Foto'])
                                            @include('components.input-file', ['name' => 'photo', 'accept' => '.jpg,.jpeg,.png', 'type' => 'JPG PNG or JPEG'])
                                        </div>
                                    </div>
                                    @include('components.button')
                                </form>
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @foreach ($photo as $item => $value)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <th class="px-6 py-4">{{ $item + 1 }}</th>
                                    <td class="px-6 py-4">
                                        <img src="{{ asset('assets/template/assets/img/portfolio/' . $value->photo) }}" class="w-10 h-10 rounded-full">
                                    </td>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah kamu yakin ingin menghapus foto?</h3>
                        <form action="{{ route('program.destroy', ['id' => $value->id]) }}" method="POST" class="inline disabled-btn">
                            @csrf
                            @method('DELETE')
            ...
            <div class="relative p-4 w-full max-w-xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="p-4 md:p-5 disabled-btn" action="{{ route('program.update', ['id' => $value->id]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="grid grid-cols-1 gap-4">
                                <div>

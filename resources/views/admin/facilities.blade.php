...
@section('id_nav', 'facilities_nav')

@section('content')
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            <x-jumbroton :page="'Fasilitas'" :url="'fasilitas'" class="mt-2" />
            <div class="grid md:grid-cols-1 gap-8">
                <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-8 md:p-12">
                    <x-tag-icon :page="'Halaman Fasilitas'" />
                    <form action="{{ route('facilities.update_hero') }}" class="disabled-btn" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                @include('components.label', ['text' => 'Judul'])
                                @include('components.input-text', ['name' => 'title', 'value' => $facilities->title, 'placeholder' => 'Judul'])
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                ...
                                @include('admin.elements.textarea-summernote', ['name' => 'description', 'value' => $facilities->description])
                            </div>
                        </div>
                    ...
                    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative p-4 w-full max-w-4xl max-h-full">
                            ...
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <form class="p-4 md:p-5 disabled-btn" action="{{ route('facilities.store') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="grid gap-4 mb-4 grid-cols-2">
                                        <div>
                                            @include('admin.elements.label', ['text' => 'Ikon'])
                                            @include('components.input-text', ['name' => 'icon', 'value' => old('icon', ''), 'placeholder' => 'icon'])

                                            <p class="text-gray-500 dark:text-gray-400">
                                                <a href="https://icons.getbootstrap.com/" target="blank" class="inline-flex items-center font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                                    Cari Ikon
                                    ...
                                    <div class="grid gap-4 mb-4 grid-cols-1">
                                        <div>
                                            @include('components.label', ['text' => 'Deskripsi'])
                                            @include('admin.elements.textarea-summernote', ['name' => 'description', 'value' => old('description', '')])
                                        </div>
                                    </div>
                                    @include('components.button')
                                </form>
                        ...
                        <tbody>
                            @foreach ($list as $item => $value)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4">{{ $value->icon }}</td>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <form action="{{ route('facilities.destroy', ['id' => $value->id]) }}" method="POST" class="inline disabled-btn">
                            @csrf
                            @method('DELETE')
            ...
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="p-4 md:p-5 disabled-btn" action="{{ route('facilities.update', ['id' => $value->id]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    @include('components.label', ['text' => 'Ikon'])
                                    @include('components.input-text', ['name' => 'icon', 'value' => $value->icon, 'placeholder' => 'Ikon'])
                                </div>
                                <div>
                                    @include('components.label', ['text' => 'Judul'])
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                <div>
                                    @include('components.label', ['text' => 'Deskripsi'])
                                    @include('admin.elements.textarea-summernote', ['name' => 'description', 'value' => $value->description])
                                </div>
                            </div>

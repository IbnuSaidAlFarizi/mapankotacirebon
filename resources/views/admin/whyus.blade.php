...
@section('title', 'Halaman Kenapa Kami')
@section('id_nav', 'whyus_nav')

@section('content')
    <section class="bg-white dark:bg-gray-800 mt-10">
        <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
            <x-jumbroton :page="'Kenapa Kami?'" :url="'why-us'" class="mt-2" />
            <div class="grid md:grid-cols-1 gap-8">
                <x-form-meta :url="'meta-why-us.update'" :meta="$meta" />
                <div class="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-8 md:p-12">
                    <x-tag-icon :page="'Halaman Beranda'" />
                    <form action="{{ route('why-us.update_hero') }}" method="POST">
                        @csrf
                        @method('PUT')
                        @csrf
                        <x-label :text="'Judul'" />
                        <x-input-text :name="'h1'" :value="$whyus->h1" :placeholder="'cth: Website KJA, KJA ABC...'" />
                        <x-label :text="'Deskripsi'" />
                        <x-textarea :name="'p'" :value="$whyus->p" :placeholder="'Ringkasan mengenai kja Anda'" />
                        <x-button />
                    </form>
                </div>
                ...
                <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 w-full max-w-md max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <!-- Modal body -->
                            <form class="p-4 md:p-5" action="{{ route('why-us.store') }}" method="POST">
                                @csrf
                                @method('POST')
                                <div class="grid gap-4 mb-4 grid-cols-2">
                                    <div class="col-span-2">
                                        <x-label :text="'Judul'" />
                                        <x-input-text :name="'title'" :placeholder="'...'" />
                                    </div>
                                    <div class="col-span-2">
                                        <x-label :text="'Deskripsi'" />
                                        <x-input-text :name="'description'" :placeholder="'...'" />
                                    </div>
                                    <div class="col-span-2">
                                        <x-label :text="'Ikon'" />
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <form action="{{ route('why-us.destroy', ['id' => $value->id]) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5">
                    <form class="p-4 md:p-5" action="{{ route('why-us.update') }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                ...
                                <x-input-text :name="'title'" :value="$value->title" :placeholder="'...'" />
                            </div>
                            <div class="col-span-2">
                                <x-label :text="'Deskripsi'" />
                                <x-input-text :name="'description'" :value="$value->description" :placeholder="'...'" />
                            </div>
                            <div class="col-span-2">
                                <x-label :text="'Ikon'" />
